	...
	case !ok:
		...
		if err1 == nil {
			if strings.Contains(path, "posixrules") {
				...
				if err1 != nil {
					framework.GetLogger(config2.KEY_FIRSTPARTY_LOG).Error("locate timezone files failed", zap.Error(err1))
					return ""
				}
			}
			...
			framework.GetLogger(config2.KEY_FIRSTPARTY_LOG).Error("infer timezone failed", zap.Error(err2))
		}
		framework.GetLogger(config2.KEY_FIRSTPARTY_LOG).Error("locate timezone files failed", zap.Error(err1))
	case tz != "" && tz != "UTC":
