...
import (
	"bytes"
	encrypt3 "github.com/pingcap-inc/tiem/library/util/encrypt"
	"io"
	"strings"
	"testing"
...
func testReadEmptyFile(t *testing.T, encrypt bool) {
	...
	var ctrCipher *encrypt3.CtrCipher
	if encrypt {
		ctrCipher, err = encrypt3.NewCtrCipher()
		if err != nil {
			return
		}
	}
	...
	for i := 0; i <= 10; i++ {
		...
		if encrypt {
			underlying = encrypt3.NewReader(underlying, ctrCipher)
		}
...
func testReadDifferentBlockSize(t *testing.T, encrypt bool) {
	...
	if encrypt {
		...
		underlying = encrypt3.NewWriter(underlying, ctrCipher)
	}
...
func testWriteDifferentBlockSize(t *testing.T, encrypt bool) {
	...
	if encrypt {
		underlying1 = encrypt3.NewWriter(underlying1, ctrCipher)
		underlying2 = encrypt3.NewWriter(underlying2, ctrCipher)
	}
...
func assertUnderlyingWrite(t *testing.T, encrypt bool, f io.WriteCloser, fc func(b []byte, offset int) []byte) (*encrypt3.CtrCipher, bool) {
...
func underlyingReadAt(f io.ReaderAt, encrypt bool, ctrCipher *encrypt3.CtrCipher, n, off int) error {
	var underlying io.ReaderAt = f
	if encrypt {
		underlying = encrypt3.NewReader(underlying, ctrCipher)
	}
...
func assertReadAtFunc(t *testing.T, encrypt bool, ctrCipher *encrypt3.CtrCipher) func(off int64, r []byte, assertErr error, assertN int, assertString string, f io.ReaderAt) {
