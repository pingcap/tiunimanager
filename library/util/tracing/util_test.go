...
import (
	"context"
	tracing2 "github.com/pingcap-inc/tiem/library/util/tracing"
	"testing"
...
func TestSpanFromContext(t *testing.T) {
	ctx := context.TODO()
	noopSp := tracing2.SpanFromContext(ctx)
	// test noop span
	...
	sp := tracing2.NewRecordedTrace("test", func(sp basictracer.RawSpan) {
		collectedSpan = append(collectedSpan, sp)
	})
	sp.Finish()
	opentracing.ContextWithSpan(ctx, sp)
	child := tracing2.SpanFromContext(ctx)
	child.Finish()
...
func TestChildSpanFromContext(t *testing.T) {
	ctx := context.TODO()
	noopSp, _ := tracing2.ChildSpanFromContxt(ctx, "")
	_, ok := noopSp.Tracer().(opentracing.NoopTracer)
	require.True(t, ok)
	...
	sp.Finish()
	opentracing.ContextWithSpan(ctx, sp)
	child, _ := tracing2.ChildSpanFromContxt(ctx, "test_child")
	child.Finish()
...
func TestFollowFrom(t *testing.T) {
	...
	sp1 := tracing2.NewRecordedTrace("test", func(sp basictracer.RawSpan) {
...
func TestTreeRelationship(t *testing.T) {
	...
	sp2, ctx := tracing2.ChildSpanFromContxt(ctx, "parent")
	sp3, _ := tracing2.ChildSpanFromContxt(ctx, "child")
