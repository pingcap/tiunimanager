...
func scaleOutCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("scale out cluster " + clusterMeta.Cluster.ID + " successfully", "version: " + cluster.Version)
	return nil
}
...
func scaleInCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("scale in cluster " + clusterMeta.Cluster.ID + " successfully")
	return nil
}
...
func freeInstanceResource(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("cluster " + clusterMeta.Cluster.ID + " recycle instance " + instanceID + " successfully")
	return nil
}
...
func clearBackupData(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	if deleteReq.KeepHistoryBackupRecords {
		...
		node.Record("keep manual backup data for cluster " + meta.Cluster.ID + " successfully")
	} else {
		...
		node.Record("delete manual backup data for cluster " + meta.Cluster.ID + " successfully")
	}
...
func backupBeforeDelete(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	if deleteReq.AutoBackup {
		...
		node.Record("do backup for cluster " + meta.Cluster.ID + " successfully")
	} else {
...
func backupSourceCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("do backup for source cluster " + sourceClusterMeta.Cluster.ID + " successfully",
...
func restoreNewCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("do restore for cluster " + clusterMeta.Cluster.ID + " successfully", "backup ID: " + backupID)
	return nil
}
...
func waitWorkFlow(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("wait workflow " + workflowID + " done")
	return nil
}
...
func setClusterFailure(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("set cluster " + clusterMeta.Cluster.ID + " status into " + string(constants.ClusterFailure) + " successfully")
	return nil
}
...
func setClusterOnline(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("set cluster " + clusterMeta.Cluster.ID + " status into " + string(constants.ClusterRunning) + " successfully")
	return nil
}
...
func setClusterOffline(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("set cluster " + clusterMeta.Cluster.ID + " status into " + string(constants.ClusterStopped) + " successfully")
	return nil
}
...
func revertResourceAfterFailure(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("recycle resource successfully")
	return nil
}
...
func persistCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("persist cluster " + clusterMeta.Cluster.ID + " successfully")
	return err
}
...
func deployCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("deploy cluster " + clusterMeta.Cluster.ID + " successfully", "version: " + cluster.Version, "yaml config: " + yamlConfig)
	return nil
}
...
func startCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("start cluster " + clusterMeta.Cluster.ID + " successfully", "version: " + cluster.Version)
	return nil
}
...
func syncBackupStrategy(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	if len(sourceStrategyRes.Strategy.BackupDate) == 0 {
		...
		node.Record("cluster " + clusterMeta.Cluster.ID + " has no backup strategy, no need to update")
		return nil
	}
	...
	node.Record("save cluster " + clusterMeta.Cluster.ID + " backup strategy successfully")
	return nil
}
...
func syncParameters(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("update cluster " + clusterMeta.Cluster.ID + " parameters successfully")
	return nil
}
...
func asyncBuildLog(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("update cluster " + clusterMeta.Cluster.ID + " build log successfully")
	return nil
}
...
func destroyCluster(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("destroy cluster: " + clusterMeta.Cluster.ID + " successfully", "version: " + cluster.Version)
	return nil
}
...
func freedClusterResource(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("cluster " + clusterMeta.Cluster.ID + " freed resource successfully")
	return nil
}
...
func initDatabaseAccount(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("cluster " + clusterMeta.Cluster.ID + " init database account successfully")
	return nil
}
...
func rebuildTopologyFromConfig(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("rebuild topology config successfully", "add instances into cluster " + clusterMeta.Cluster.ID + " topology successfully")
	return nil
}
...
func testConnectivity(node *workflowModel.WorkFlowNode, context *workflow.FlowContext) error {
	...
	node.Record("test connect TiDB address " + connectAddress.IP + ":" + strconv.Itoa(connectAddress.Port) + "successfully")
	return nil
}
