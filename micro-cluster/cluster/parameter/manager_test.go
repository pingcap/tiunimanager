	...
	t.Run("inspect value equals", func(t *testing.T) {
		inspectApiParams, instDiffParams, err := inspectApiParameter(context.TODO(), instances[0], apiContent, []structs.ClusterParameterInfo{
			...
			},
			{
				ParamId:      "5",
				Category:     "raftstore",
				Name:         "peer-stale-state-check-interval",
				InstanceType: "TiDB",
				Type:         1,
				UnitOptions:  []string{"s", "m"},
				RangeType:    1,
				RealValue:    structs.ParameterRealValue{ClusterValue: "300s"},
			},
			{
				ParamId:      "6",
				Category:     "coprocessor",
				Name:         "region-max-keys",
				InstanceType: "TiDB",
				Type:         0,
				RealValue:    structs.ParameterRealValue{ClusterValue: "1440000"},
			},
			{
				ParamId:      "7",
				Category:     "rocksdb",
