...
func (mgr *ImportExportManager) ExportData(ctx context.Context, request message.DataExportReq) (resp message.DataExportResp, exportErr error) {
	...
	exportDir := filepath.Join(exportPrefix, request.ClusterID, fmt.Sprintf("%s_%s", exportTime.Format("2006-01-02-15-04-05"), request.StorageType))
...
func (mgr *ImportExportManager) ImportData(ctx context.Context, request message.DataImportReq) (resp message.DataImportResp, importErr error) {
	...
	importDir := filepath.Join(importPrefix, request.ClusterID, fmt.Sprintf("%s_%s", importTime.Format("2006-01-02-15-04-05"), request.StorageType))
	if request.RecordId == "" {
