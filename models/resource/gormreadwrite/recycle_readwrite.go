...
func (rw *GormResourceReadWrite) recycleHolderResource(ctx context.Context, tx *gorm.DB, holderId string) (err error) {
	...
	if err != nil {
		return framework.NewTiEMErrorf(common.TIEM_RESOURCE_SQL_ERROR, "get cluster %s total used compute failed, %v", holderId, err)
	}

	var usedDisks []string
	err = tx.Model(&mm.UsedDisk{}).Select("disk_id").Where("holder_id = ?", holderId).Scan(&usedDisks).Error
	if err != nil {
		return framework.NewTiEMErrorf(common.TIEM_RESOURCE_SQL_ERROR, "get cluster %s total used disks failed, %v", holderId, err)
	}
...
func (rw *GormResourceReadWrite) recycleUsedTablesByHolder(ctx context.Context, tx *gorm.DB, holderId string) (err error) {
	...
	if err != nil {
		return framework.NewTiEMErrorf(common.TIEM_RESOURCE_SQL_ERROR, "recycle UsedCompute for cluster %s failed, %v", holderId, err)
	}

	err = tx.Where("holder_id = ?", holderId).Delete(&mm.UsedDisk{}).Error
	if err != nil {
		return framework.NewTiEMErrorf(common.TIEM_RESOURCE_SQL_ERROR, "recycle UsedDisk for cluster %s failed, %v", holderId, err)
	}

	err = tx.Where("holder_id = ?", holderId).Delete(&mm.UsedPort{}).Error
	if err != nil {
		return framework.NewTiEMErrorf(common.TIEM_RESOURCE_SQL_ERROR, "recycle UsedPort for cluster %s failed, %v", holderId, err)
	}
	return nil
}
