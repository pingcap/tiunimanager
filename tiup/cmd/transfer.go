...
func newPullCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:  "(EXPERIMENTAL) Transfer files or directories from host in the tiem cluster to local",
		Hidden: true,
...
func newPushCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:  "(EXPERIMENTAL) Transfer files or directories from local to host in the tiem cluster",
		Hidden: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) != 3 {
				return cmd.Help()
