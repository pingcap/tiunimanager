...
import (
	"bytes"
	checksum2 "github.com/pingcap-inc/tiunimanager/util/checksum"
	"io"
	"os"
	"testing"
...
func (s *testAesLayerSuite) TestReadAt(c *check.C) {
	...
	readAtTestCases := []readAtTestCase{
		...
		{
			newWriter: func(f *os.File) io.WriteCloser { return checksum2.NewWriter(NewWriter(f, ctrCipher1)) },
			newReader: func(f *os.File) io.ReaderAt { return checksum2.NewReader(NewReader(f, ctrCipher1)) },
		},
		{
			newWriter: func(f *os.File) io.WriteCloser { return NewWriter(checksum2.NewWriter(f), ctrCipher1) },
			newReader: func(f *os.File) io.ReaderAt { return NewReader(checksum2.NewReader(f), ctrCipher1) },
		},
		{
...
func BenchmarkReadAt(b *testing.B) {
	...
	readAtTestCases := []readAtTestCase{
		...
		},
		{
			...
			newWriter: func(f *os.File) io.WriteCloser { return checksum2.NewWriter(f) },
			newReader: func(f *os.File) io.ReaderAt { return checksum2.NewReader(f) },
		},
		{
			name: "data->checksum->encrypt->file",
			newWriter: func(f *os.File) io.WriteCloser {
				return checksum2.NewWriter(NewWriter(f, ctrCipher))
			},
			newReader: func(f *os.File) io.ReaderAt {
				return checksum2.NewReader(NewReader(f, ctrCipher))
			},
		},
	}
